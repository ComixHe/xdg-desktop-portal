...
static XdpAppInfo *xdp_app_info_linyaps_new_testing(const char *sender,
                                                    GError **error) {
  ...
  ret =
      g_key_file_load_from_file(metadata, metadata_path, G_KEY_FILE_NONE, NULL);
  g_assert(ret == TRUE);

  app_id = g_key_file_get_string(metadata, LINYAPS_METADATA_GROUP_APPLICATION,
                                 LINYAPS_METADATA_KEY_APP_ID, error);
  g_assert(app_id != NULL);
  instance_id = g_key_file_get_string(metadata, LINYAPS_METADATA_GROUP_INSTANCE,
                                      LINYAPS_METADATA_KEY_INSTANCE_ID, error);
  g_assert(instance_id != NULL);
  network = g_key_file_get_string(metadata, LINYAPS_METADATA_GROUP_CONTEXT,
                                  LINYAPS_METADATA_KEY_NETWORK, error);
  ...
  app_info_linyaps = g_initable_new(
      XDP_TYPE_APP_INFO_LINYAPS, NULL, error, "engine", LINYAPS_ENGINE_ID,
      "flags", flags, "id", app_id, "instance", instance_id, "sender", sender, NULL);

  return XDP_APP_INFO(g_steal_pointer(&app_info_linyaps));
}

/* this implementation refers to flatpak */
static int open_linyaps_info(int pid, GError **error) {
  g_autofree char *root_path = NULL;
  g_autofd int root_fd = -1;
  g_autofd int info_fd = -1;

  root_path = g_strdup_printf("/proc/%u/root", pid);
  root_fd = openat(AT_FDCWD, root_path,
...
static void xdp_app_info_linyaps_dispose(GObject *object)
{
  G_OBJECT_CLASS(xdp_app_info_linyaps_parent_class)->dispose(object);
}

static void xdp_app_info_linyaps_class_init(XdpAppInfoLinyapsClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  object_class->dispose = xdp_app_info_linyaps_dispose;
}

static void xdp_app_info_linyaps_init(XdpAppInfoLinyaps *app_info_linyaps)
{
}

XdpAppInfo *xdp_app_info_linyaps_new(const char *sender, int pid, int *pidfd,
                                     GError **error) {
  g_autoptr(XdpAppInfoLinyaps) app_info_linyaps = NULL;
  g_autofd int fd = -1;
  ...
  app_info_linyaps =
      g_initable_new(XDP_TYPE_APP_INFO_LINYAPS, NULL, error, "engine",
                     LINYAPS_ENGINE_ID, "flags", flags, "id", app_id, "instance", instance_id, "sender", sender, NULL);
